...
export const FighterCard = ({ llm, isSelected, onClick, disabled, selectionStep }: FighterCardProps) => {
    ...
    const getImagePath = (llmType: LLMType) => {
        const mapping: Record<string, string> = {
            GPT4o: "/assets/tests/openai/GPT4o_256.png",
            Claude: "/assets/tests/openai/claude.png",
            Grok: "/assets/tests/openai/grok.png",
        };
...
export const FighterPortrait = ({ llm, side, isPlayerOne }: FighterPortraitProps) => {
    ...
    const getImagePath = (llmType: LLMType | undefined) => {
        ...
        const mapping: Record<string, string> = {
            GPT4o: "/assets/tests/openai/GPT4o_512.png",
            Claude: "/assets/tests/openai/claude.png",
            Grok: "/assets/tests/openai/grok.png",
        };
        ...
        {fighters.map(fighter => (
            <Col xs={12} sm={8} md={6} key={fighter}>
                <FighterCard
                    ...
                    selectionStep={
                        (fighter === selectedFighter1) ? 1 : 
                        (fighter === selectedFighter2) ? 2 : 
                        selectionStep
                    }
                />
            </Col>
...
export const GameBackground = styled.div`
  ...
  right: 0;
  bottom: 0;
  background-image: linear-gradient(to right, ${themeColors.border} 1px, transparent 1px), 
...
export const GameTitle = styled(Title)`
  ...
  padding: 20px 0;
  font-weight: 900;
  text-shadow: 0 0 10px ${themeColors.accent}, 0 0 20px rgba(255, 62, 0, 0.5);
  position: relative;
...
export const FighterCardContainer = styled(Card)<{ 
  ...
  border-color: ${props => props.$isSelected ? 
    (props.$selectionStep === 1 ? themeColors.accentBlue : themeColors.accent) : 
    'transparent'};
  border-width: ${props => props.$isSelected ? '2px' : '0'};
  box-shadow: ${props => props.$isSelected ? 
    (props.$selectionStep === 1 ? themeColors.glowBlue : themeColors.glowAccent) : 
    'none'};
...
export const PortraitContainer = styled(Card)<{
  ...
  text-align: center;
  border-color: ${props => props.$hasLlm ? 
    (props.$isPlayerOne ? themeColors.accentBlue : themeColors.accent) : 
    themeColors.border};
  border-width: ${props => props.$hasLlm ? '2px' : '1px'};
  box-shadow: ${props => props.$hasLlm ? 
    (props.$isPlayerOne ? themeColors.glowBlue : themeColors.glowAccent) : 
    'none'};
...
export const PortraitName = styled(Title)<{
  ...
  margin: 0;
  color: ${props => props.$hasLlm ? 
    (props.$isPlayerOne ? themeColors.accentBlue : themeColors.accent) : 
    themeColors.text};
  text-transform: uppercase;
  letter-spacing: 2px;
  text-shadow: ${props => props.$hasLlm ? 
    `0 0 10px ${props.$isPlayerOne ? 'rgba(0, 180, 255, 0.5)' : 'rgba(255, 62, 0, 0.5)'}` : 
    'none'};
`;
